...
- **`deconstruct.py`** — the inverse pipeline. CIF/Structure → EconNN bonds capped at `BOND_LENGTH_SLACK` × UFF radius sums (EconNN is adaptive and bonds isolated guests to anything) → 0-periodic components removed as guests → metal-oxo clustering (metals + C-free metal-bound atoms + carboxylate/sulfonate/phosphonate groups — cuts fall at carboxylate-C↔backbone-C and metal↔donor bonds, matching the shipped SBU dummy convention) → dummy-capped `Fragment`s (X at cut-bond midpoints) → quotient graph (reuses `framework_quotient_edges` with unit ids as slot provenance) → `identify_net`. Metal-free structures and rod (1-periodic) units raise `DeconstructionError`. Entry points: `Autografs.deconstruct(source)`, module-level `deconstruct(source, topologies=...)`.
