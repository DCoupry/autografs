...
from autografs.utils.operations import rotation
from autografs.utils.operations import reflection
from autografs.utils.operations import inertia
from autografs.utils.operations import is_valid_op
...
class PointGroup(object):
    ...
    def analyze(self):
        ...
        if len(self.mol) == 1:
            ...
            eig_diff = (abs(e0 - e1) > self.etol) & (abs(e0 - e2)
                                                     > self.etol) & (abs(e1 - e2) > self.etol)
            logger.debug(
                "Inertia tensor eigenvalues = {0:3.2f} {1:3.2f} {2:3.2f}".format(
                    *self.eigvals))
    ...
    def find_possible_equivalent_positions(self, axis=None):
        ...
        dists = numpy.linalg.norm(
            self.mol.get_positions(), axis=1, keepdims=True)
        ...
        for cval in set(clusters):
            ...
            indices = indices[numpy.where(
                numbers[indices] == numpy.argmax(counts))[0]]
            if axis is not None:
